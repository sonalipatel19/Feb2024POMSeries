...
public class JavaScriptUtil {
	...
	public void goBackWithJS() {
		js.executeScript("history.go(-1)"); //history.back()
	}
	
	public void goForwardWithJS() {
		js.executeScript("history.go(1)"); //history.forward()
	}
	 ...
	 */
	public void zoomChromeEdgeSafari(String zoomPercentage) {  // to check the responsiveness of the application.
		String zoom = "document.body.style.zoom = '"+zoomPercentage+"%'";
		js.executeScript(zoom);
	}
	
	public void zoomFirefox(String zoomPercentage) {  // to check the responsiveness of the application.
	...
	public void flash(WebElement element) {
		...
		for(int i = 0; i < 10; i++) {
			...
			changeColor(bgcolor, element); //greywhite
		}
	}
